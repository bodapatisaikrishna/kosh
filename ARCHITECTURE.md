...
**The result, independently checked against `ground_truth.json` directly rather than trusted from the pipeline's own summary** ([`benchmarks/phase5_live_residual.json`](benchmarks/phase5_live_residual.json), traces in [`benchmarks/sample_traces_live/`](benchmarks/sample_traces_live/)): 2 of 6 records correctly matched — both cross-checked by hand against `ground_truth.json`'s `order_to_payment` links, not just read off `false_match_rate: 0.0` — and 4 of 6 ended `AGENT_INCOMPLETE`, exhausting the 8-turn budget on genuinely hard cases rather than guessing. Zero false matches, same as every other layer. The 4-of-6 incomplete rate is reported as a real limitation (see README), not smoothed over — the turn budget or the prompting has room to improve on a small sample this hard.

**A second real gap, found while checking this run against Phase 5's own "cost < $0.50/1000 records" checkpoint**: there was no number to check. `EngineMeta.cost_usd_micros` existed in the contract and was threaded through `eval/metrics.py`'s `cost_per_1000_records` calculation, but nothing ever *set* it — every committed benchmark, including this real run with 115,607 real tokens billed against a real account, reported `cost_usd_micros: 0`. Token counts were tracked correctly throughout; only the USD conversion was missing, silently, since Phase 5. Fixed with `engine/llm/pricing.py`: a small per-model $/1M-token table (NIM's rate for `nvidia/nemotron-3-ultra-550b-a55b` sourced from its public OpenRouter listing, since NVIDIA doesn't publish per-token pricing for the hosted NIM preview endpoint itself; Anthropic's official published rates included for `AnthropicClient`, never run live) feeding a `cost_usd_micros(model, input_tokens, output_tokens)` function, wired through `l3_agent.py`'s per-trace and aggregate cost, `pipeline.py`'s meta construction, and a new headline card in `eval/report.py` (shown only when `llm_calls > 0`, so a deterministic-only run never shows a misleading "$0.00"). Recomputed retroactively from the already-recorded token counts — no new API calls needed. The real number: **$0.0749 for the live run, $0.0403 per 1000 records** — comfortably inside the brief's target, but the point isn't that it passed, it's that it couldn't have been checked at all before this fix.
