...
## Hardening sprint, Task 2: the compound-leg category coercion

L3 could correctly find that a payment's fee tier AND tax rate were both wrong at once (`compound_fee_tax_error` - two overlapping causes, so no single-cause hypothesis decomposes it), but had no structural way to say so: it labelled the record with whichever single category it asked for (`FEE_VARIANCE`), citing whichever leg's own delta happened to be largest - even when that leg's delta wasn't the actual net amount at risk (fee and GST deltas partially offsetting means the real bottom line is smaller than either leg alone). Per-class scoring on the live residual: 1 detected / 4 misclassified / 1 missed.

**Fixed structurally, in the tool layer, not the prompt** - the same "recompute, don't trust" pattern as `severity_for_amount`. `engine/l3_tools.py` gained: a `VarianceObservation` recorded on every `explain_variance` call (observed/expected/delta/cause); `_unexplained_legs(ctx)`, deduped on `(observed, expected)` so a model re-checking itself with an identical call never inflates the count; `_bottom_line_leg(legs)`, the leg with the largest `abs(expected_paise)` - the net-level comparison, not the largest delta. In `raise_exception`, if 2+ distinct undecomposable legs are on record, the category is coerced to `UNEXPLAINED_VARIANCE` and the amount to the true bottom-line delta, with the coercion and every leg logged into the evidence chain - never silent. `PROMPT_VERSION` bumped 1→2 first, so the cache-busting this requires actually happens (the exact trap `max_turns` hit once before).

**7 new tests**, the false-positive guard written first: one unexplained leg alone must never coerce (over-firing would turn every genuine single-cause `FEE_VARIANCE` into a vague `UNEXPLAINED_VARIANCE` and cost accuracy on a class that scores perfectly today). Deterministic suite re-verified byte-identical against every committed benchmark before spending anything on a live re-run - L0/L1/L2 are untouched by this change.

**Two live runs, both real, both fully verified against ground truth, genuinely different results - and that's the honest finding.** The first (36 minutes, a badly degraded NIM endpoint that day - see below) scored `compound_fee_tax_error` **6/6 detected**: the model investigated thoroughly enough on every record to surface 2+ legs each time. A second run minutes later scored **4 detected / 1 misclassified / 1 missed**: reading the misclassified record's own trace, `unexplained_leg_count: 1` - the model checked only the GST leg this time and never separately checked the fee leg, so the coercion correctly did not fire on one data point. This is not a flaw in the fix; it's the false-positive guard working exactly as designed, against genuine run-to-run variance in how thoroughly a live model chooses to investigate. All 6 amounts were exact to the paisa against the generator's own labelled defect in *both* runs, and zero false matches in either. `benchmarks/phase5_live_residual.json` (the second, fully-clean run) is the committed reference; its `compound_fee_tax_error_note` field records the first run's 6/6 result for comparison.

**Two more real bugs, found while chasing an unrelated anomaly during Task 3, both resolved before either live run above was trusted:**

1. **A first attempt at Task 3's "attack f" (see below) broke `settlement_split`, caught immediately, reverted.** Not part of Task 2's own scope, but discovered and fixed in the same window - see the next section.
2. **The first live run of *this* task's own fix showed a `settlement_split` anomaly** (`false_exception_raised: 11` instead of `correctly_resolved: 11`) that survived extensive isolation testing: `git stash` back to the true pre-sprint commit plus `client=None` gave 11/11 correct; a scripted `FakeClient` closing the loop immediately gave 11/11 correct; three fresh-process repeats gave 11/11 correct every time. The anomaly did **not** reproduce in the second live run (11/11 correct there). Documented rather than chased further: observed once, in one 36-minute run against a degraded endpoint with unusually high retry/backoff activity, never reproduced in three separate controlled tests or a second live run. If it recurs, the isolation tests above are the starting point, not a fresh investigation.

## Hardening sprint, Task 3: the settlement double-claim guard

Adversarial attack (Task 3, attack f): the identical UTR text appears in two separate bank rows. `engine/l0_deterministic.py::match_settlement_bank_txn` iterated every bank row independently with no check at all for whether a settlement had already been claimed by an earlier row in the same pass - it happily matched **both** rows to the same settlement, which would double-count that settlement's cash in any real reconciliation.

**The first fix was wrong, and broke a real feature - caught immediately, not shipped.** The obvious guard ("a settlement claimed once can't be claimed again") was implemented in both L0 and L1 (which had the identical gap one layer downstream, never having been told which settlements L0 already claimed). Running the full suite immediately after: 4 failures, all in the cash-reconciliation identity test. Root cause: `settlement_split` (11/11 correctly resolved, a real and already-correct feature) deliberately puts the *same* UTR on two bank rows on purpose - `data/generator/defects.py::_inject_settlement_split` splits one settlement's payout into two genuine partial deposits (`part_a`/`part_b` summing exactly to `net_paise`), both carrying the settlement's own UTR, because that's what a real split payout looks like on a bank statement. The one-claim-only guard couldn't tell that apart from a true collision and refused the second, legitimate half every time - `unidentified_credit_paise` on `run_2000` jumped from ₹1,20,449.00 to ₹4,39,856.37, a ~3.65x inflation, caught by `test_reconciliation_ties_exactly_on_run_2000` failing outright. Reverted via `git checkout` within the same session, before it was ever committed.

**The correct fix is an accounting check, not a claim-count check**, in `engine/pipeline.py::_reconcile_settlement_credit_sums` - a post-pass over every `settlement_bank_txn` match together, run once after L0+L1+L2 have all contributed (none of the three layers can see the others' matches while running, so this is the first point anything can check one settlement's full picture at once). A settlement's linked credits summing to at most its own `net_paise` (+₹1 tolerance, for paisa-level rounding elsewhere in the chain) is accepted regardless of how many bank rows that is - correctly allowing `settlement_split`. A sum *exceeding* `net_paise` means something is genuinely wrong, and every link for that settlement is refused rather than guessing which one is real - same "never guess" principle as every ambiguity guard inside a single layer, just applied across all three layers' combined output.
...
## Hardening sprint, Task 4: the all-LLM ablation, and a real hang it surfaced

The point of Task 4 was to measure, not assert, that deterministic-first is the right architecture: route every payment/settlement/bank record straight to L3 (`engine/pipeline.py::run_llm_only`), bypassing L0–L2 *and* L4 entirely, and see what an all-LLM system actually costs and scores against the same fixture. On `data/fixtures/sample_200` that's 348 investigations (202 payments + 71 settlements + 75 bank rows), not the ~6-record residual the real pipeline ever sends to L3.

**Two crashes, found by being the first mode that ever routes a "failed" payment (empty `captured_at`) to L3 at all** - `run_llm_only` is a materially different caller than the real pipeline, which never sends a never-captured payment to L3 in the first place (L0/L4 handle it first). `_residual_aging_days` (l3_tools.py) and its twin `_aging_days_hint` (l3_agent.py) both called `aging_days(as_of, event_date)` on an empty string and raised `ValueError` - and critically, `_aging_days_hint` is called from *inside* `run_one`'s own except-block, the fallback path that exists specifically to catch and isolate a failing record without taking down the batch. A safety net that itself throws on the exact input class it's supposed to handle isn't a safety net. Fixed by making both functions return `0` gracefully on an empty `event_date`; `find_candidates` got the same treatment (raises a proper `ToolError` instead of crashing when the record under investigation itself has no `captured_at`, plus a truthy-guard in its bank-debit candidate loop). Four regression tests added across `tests/test_l3_tools.py` and `tests/test_l3_agent.py`.

**A third, more serious gap, found live**: the first full-batch attempt hung for **10+ hours with zero progress and no error** after the laptop it ran on went to sleep overnight. `lsof` on the stuck process showed all 8 `DEFAULT_CONCURRENCY` connections `ESTABLISHED` and 0% CPU - every worker thread's `client.complete()` call was blocked on a TCP connection that had gone half-open across the sleep, and the openai SDK's own 600s read timeout never fired because nothing ever arrived to time out *against*. `run_one`'s existing per-record isolation (`engine/l3_agent.py`) only protects the batch once an upstream call actually raises - it does nothing for a call that never returns at all, and with every one of the 8 concurrency slots wedged the same way, the whole `asyncio.gather` simply stopped moving. Fixed with an outer `PER_RECORD_TIMEOUT_SECONDS = 900.0` ceiling via `asyncio.wait_for` around each record's investigation, proven with a regression test that a fake client hanging on one record times out into an honest `AGENT_INCOMPLETE` without blocking its sibling record's completion (`test_a_hung_client_call_times_out_instead_of_stalling_the_whole_batch`). Disclosed, not hidden: `asyncio.wait_for` cancels the *await*, not the underlying OS thread (Python can't force-kill a thread), so `asyncio.run()`'s own cleanup can still be slow to return if a thread is blocked forever rather than just past the ceiling - a materially better failure mode than before (every healthy record still completes and is persisted to disk on time) but not an absolute guarantee against every possible hang.

**A mistake made investigating it, corrected rather than glossed over**: after finding the above and writing the fix, the process was killed on the strength of the last-known snapshot (139 records, unchanged since the sleep) without re-checking freshness immediately before acting. By the time the kill actually happened, the run had already recovered on its own and reached 159 records at its fastest pace of the whole run (~2.5 records/min) - a healthy, fast-progressing process was killed on stale evidence. No data was lost (every trace and its cache entry is written to disk as it completes, so the 159 already-done records were simply reused on the next run), and the timeout fix above is independently correct regardless of whether this specific run turned out to have actually been stuck - but the kill itself was premature, and the fix for *that* is procedural, not code: verify freshness (a fresh timestamp on the most recent file, not just a raised count) immediately before any destructive action, not from a read that's already several minutes old by the time the decision is acted on.

**The result, on the second (resumed, cache-hit) run**: 348/348 records, zero crashes, wall clock ~53 minutes, 1,207 LLM calls. **81.52% auto-match, 0.00% false-match, 100% precision, 77.36% recall**, at $6.58 total ($35.76/1000 records) - see the README's ablation table for the full comparison against L0+L1, L0+L1+L2, and full. False-match rate holding at 0.00% even with every deterministic layer disabled is the same "refuse rather than guess" discipline (constraint 3: confidence < 0.85 → `raise_exception`) doing its job under load; the recall drop is that discipline correctly declining to guess on records the deterministic layers would have resolved with certainty. Source: `benchmarks/ablation_llm_only.json`.

## Input validation (hardening sprint, Task 5)

`engine/io.py::load_dataset` did zero validation before this: every field was a raw `int(r["..."])` or `r["..."]` dict lookup, so a missing column raised an uncaught `KeyError`, a non-integer amount an uncaught `ValueError`, both with Python's default traceback and no file/row/field context - and `csv.DictReader` doesn't raise on a duplicate header (a later column silently clobbers an earlier one) or on a row with more fields than the header (the overflow lands under a `None` key, silently, unless something checks for it). A real system ingests files produced by other systems - a bank export, an ERP dump - and those break in ways a synthetic generator never will.

**Wrapped, not replaced**: `load_dataset`'s public signature and return type are unchanged, so every existing call site keeps working. Each of the four CSVs now goes through a small per-file validation pass before constructing its rows: header validated first (missing columns, duplicate columns, or a genuinely empty file with no header at all), then a data-rows check (a header with zero rows is itself rejected - a merchant's payments file being unexpectedly empty is a real ingestion red flag worth failing loudly on, not silently reconciling "nothing" and reporting a misleading 100% match rate), then per-row checks: field-overflow (`csv.DictReader`'s `None`-key convention), integer parsing, non-negativity where the field's real-world semantics require it, date/datetime format, and duplicate primary keys. Every problem raises `DatasetValidationError(filename, reason, row, field)` - never a silent skip, never a coerced value. Fails at the *file* boundary, not the first bad row anywhere: a whole file is cheap to fully scan, so if row 47 and row 48 are both broken it's more useful to see both than to stop at 47 - but the first file with any problem stops the whole load, rather than reading all four and reporting a confusing partial picture.

**Two real, if minor, findings surfaced while deciding exactly which fields "only positive is valid" actually applies to** - checked empirically against the generator and every committed fixture, not assumed:

- A `payment.captured_at` of empty string is not a malformed date - it's the correct value for a `status="failed"` payment, which never captured and so genuinely has no capture timestamp. The date-format check only fires on a non-empty, unparseable value.
- `bank.credit_paise`/`debit_paise` are deliberately **not** validated non-negative, despite the task's own working example being exactly "negative credit where only positive is valid." A defect injector in `data/generator/defects.py` applies a net-adjustment delta directly to a settlement's linked bank credit (`linked[0].credit_paise += delta_paise`), and a large-enough negative delta on an already-small credit can drive it below zero - confirmed empirically: none of the four committed reference fixtures (`run_500`/`run_2000`/`run_10000`/`sample_200`) hit this, but seed 100 in the Task 1 multi-seed sweep produced a real `credit_paise: -34800`. Enforcing non-negativity here would have made a real, current generator output fail to load. Flagged as a separate generator-side finding (a negative credit doesn't correspond to anything a real bank statement would show) rather than silently working around it in the validation layer - `order.gross_paise`/`payment.gross_paise`/`settlement.gross_paise` are validated non-negative instead, checked clean across all four fixtures first. `settlement.adjustment_paise` is deliberately never checked either - it's a genuinely signed figure by design (a gateway recovery or credit can be negative), same principle as `aging_days`'s own "know the domain before hard-coding a rule" discipline elsewhere in this codebase.

12 tests in `tests/test_malformed_input.py` cover all 10 required cases (missing column, duplicate header, non-integer amount, negative-where-only-positive-valid, unexpected date format, empty file, headers-only file, non-UTF8 bytes, duplicate primary key, row with more fields than header) plus one confirming a clean fixture set still loads unchanged, and one confirming the failed-payment/empty-`captured_at` case specifically is not a false positive. All 8 committed benchmark/baseline reports regenerated and diffed byte-for-byte afterward - purely additive, zero drift.
