...
## 12. Post-submission stretch goals

Everything above (§§1–11) is the submission: code-frozen, benchmarked, judge-fix-verified, publicly pushed. This section covers work done *after* that point, at the user's explicit request for "the best project with all included" — specifically the two optional items the original brief allowed but the shipped submission didn't build (`README.md`'s own Limitations: *"No FastAPI layer and no live/interactive dashboard — the static HTML report is the deliverable, per the brief's own explicit fallback preference"*). Recorded separately from §11 because it's a deliberate scope extension, not a continuation of the hardening sprint or a freeze violation — no `engine/*.py` file was touched to build it.

### 12.1 — FastAPI layer + interactive Next.js dashboard

**What was built**: `api/` (FastAPI: a live-triggerable run endpoint, historical-benchmark endpoints, and static mounts for the trace files the dashboard links to) and `dashboard/` (Next.js + Recharts, matching the brief's own named stack), both local-only by explicit decision — no hosting, no public exposure of a compute-triggering endpoint.

**Design discipline carried over from the rest of this project**: the API is a thin wrapper around code that already exists and is already trusted — `data.generator.generate.run` and `eval.report.run_eval` — not a second implementation. `tests/test_api.py` asserts this directly: a live run at `seed=42/records=2000/engine=full` produces output byte-for-byte identical (modulo timing/manifest) to the committed `freeze_2000.json`.

**A real bug caught before shipping, in the same spirit as everything else in this log**: the first version generated live runs into a randomly-named temp directory. `eval/report.py`'s trace-drilldown lookup (`SAMPLE_TRACE_SOURCES`, from the earlier judge-fix round) matches on the fixture directory's *name* being `"run_2000"`, not its content — so a live `seed=42` run would reproduce `run_2000`'s data exactly, yet its exception drill-down links would all silently come back empty, because the scratch directory had the wrong name. Fixed by naming the scratch directory `run_<records>` — the same convention a real fixture at that scale would use. Verified against the actual running API, not just pytest: a live `seed=42` run's `UNEXPLAINED_VARIANCE` exceptions link to the real, already-committed agent traces in `benchmarks/sample_traces_live/`, and a `freeze_10000` view still shows zero trace links through the dashboard — the same fixture-scoping collision guard proven in the earlier judge-fix commit, now proven again through a second consumer.

**Safety**: `api/runs.py::ENGINE_ALLOWLIST` = `(null, oracle, l0l1, l0l1l2, full)` — the costed `llm-only` engine is asserted absent from it at import time, and no request field can supply an LLM client, model, or key. Tested directly (`test_the_costed_engine_is_never_reachable_from_the_api`), not just implied by the allowlist's existence. `GET /api/benchmarks/{name}` uses an explicit filename allowlist, never a path built from request input — tested against literal traversal attempts, which 404 like any other unknown name.
